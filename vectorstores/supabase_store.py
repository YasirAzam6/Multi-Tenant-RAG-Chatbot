...
class SupabaseVectorDB:
    ...
    def search(self, organization_id: str, query_embedding: List[float], top_k: int = 5):
        """
    ...
    def get_all_documents(self, organization_id: str):
        """
        Retrieve all documents for a given tenant.
        """
        response = self.client.table(self.table).select("*").eq("organization_id", "org_" + organization_id).execute()
        return response.data
